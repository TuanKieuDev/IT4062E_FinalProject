...
static Active_user user[MAX_USER];
static Group group[MAX_GROUP];

static Public_key_users pub[512];
...
void handle_login(int conn_socket)
{
    ...
    target_acc = account_find(username);
    // Of course this can be optimised into a chain of ternary operations...
    if (!target_acc)
    {
        result = INCORRECT_ACC;
    }
    else if (strcmp(target_acc->password, password) != 0)
    {
        result = INCORRECT_ACC;
    }
    else if (target_acc->is_signed_in)
    {
...
void sv_user_use(int conn_socket)
{
    ...
    for (; i < MAX_USER; i++)
    {
        ...
        {
            account_logout(user[i].username);
            user[i].socket = -1;
            for (int j = 0; j < MAX_GROUP; j++)
            {
...
void save_public_key(char *sender, char *msg)
{
    ...
    printf("Public Key of %s:\n Modulus: %lld\n Exponent: %lld\n\n",
           sender,
           (long long)pub[i].public_key->modulus,
           (long long)pub[i].public_key->exponent);
}
...
{
    strcpy(pkg->msg, "FEATURE: GROUP CHAT\n");
    send(conn_socket, pkg, sizeof(*pkg), 0);
}
...
void sv_invite_friend(int conn_socket, Package *pkg)
{
    ...
    if (friend_id >= 0)
    {
        ...
        else // okey dokey
        {
            ...
            sprintf(pkg->msg, "\"%s\" has added \"%s\" into the group.", user[user_id].username, user[friend_id].username);
